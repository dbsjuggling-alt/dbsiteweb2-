...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    try {
      ...
      // Shipping details: new API format (collected_information) then classic
      ...
      // 3. Send shipping notification if tracking available
      if (trackingNumber) {
        ...
        console.log('[webhook] Shipping notification sent');
      }
